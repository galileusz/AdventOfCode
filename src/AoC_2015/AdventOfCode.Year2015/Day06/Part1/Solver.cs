...
namespace AdventOfCode.Year2015.Day06.Part1;

internal class Solver : BaseResolver, IAdventOfCode
{
	private readonly Dictionary<Position, Light> _lightsDictionary;

	public Solver()
	{
		_lightsDictionary = new Dictionary<Position, Light>();
		...
		for (int x = 0; x < 1000; x++)
		{
			for (int y = 0; y < 1000; y++)
			{
				...
				_lightsDictionary[position] = new Light(x, y);
			}
		}
	}
	...
	public override string Solve(string input)
	{
		...
		return _lightsDictionary.Count(x => x.Value.IsOn).ToString();
	}
